      ...
      if(root != null){
        ...
        //TODO: replace true the condition in which we have to move right
          ...
          if(result == null){ //couldn't find a predecssor;
            BinaryNode<T> parent = root; // The current root might be the right ancestor
            while(parent != null && entry.compareTo(parent.data) <= 0){
              parent = parent.right; // Move up to find the right ancestor
            }
            result = parent;
          }
      ...
      if(root != null){
        ...
        if(compareResult == 0){ //found the node;
          ...
          if(result == null){ //couldn't find a successor;
            BinaryNode<T> parent = root; // The current root might be the left ancestor
            while(parent != null && entry.compareTo(parent.data) >= 0){
              parent = parent.left; // Move up to find the left ancestor
            }
            result = parent;
          }
        }
